...
const JoinBattlePage: React.FC = () => {
  ...
  const { createNewDot, isRegistrationOpen, nextSimulationTime } = useGame();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isRegistrationOpen) {
      setFormStatus('error');
      setErrorMessage('Registration is currently closed. Please wait for the current battle to end.');
      return;
    }
    ...
    try {
      const success = await createNewDot(dotName);
      ...
      if (success) {
        ...
        setFormStatus('error');
        setErrorMessage('This name is already taken or the maximum number of dots has been reached.');
      }
    } catch (error) {
      setFormStatus('error');
      ...
      <div className="container mx-auto px-4">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="bg-gray-800 bg-opacity-70 rounded-lg p-6 md:p-8 border border-gray-700 shadow-xl">
            {/* Registration status */}
            <div className={`mb-6 p-4 rounded-lg ${isRegistrationOpen 
              ...
              <div className="flex items-center">
                ...
                <span className={isRegistrationOpen ? 'text-green-400' : 'text-red-400'}>
                  {isRegistrationOpen 
              ...
              {nextSimulationTime && (
                <div className="mt-2 text-sm text-gray-400">
                  Next battle starts: {nextSimulationTime.toLocaleDateString()} at {nextSimulationTime.toLocaleTimeString()}
                </div>
              )}
            </div>
            ...
            {formStatus === 'success' ? (
              ...
              <form onSubmit={handleSubmit}>
                <div className="mb-6">
                  <h2 className="text-xl font-semibold text-white mb-4">Register Your Dot</h2>
                  ...
                  <div className="text-xs text-gray-400 mt-1 mb-6">
                    "Dot_" will be automatically added to your name if you don't include it.
                  </div>
                ...
                >
                  {formStatus === 'submitting' ? 'Registering...' : 'Register for Battle'}
                </Button>
                
                <div className="mt-6 text-center text-sm text-gray-400">
                  <p>
                    By registering, you agree to our imaginary terms of service and confirm that your dot is ready to battle!
                  </p>
                </div>
              </form>
            )}
          ...
          <div className="mt-10 text-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-gray-800 bg-opacity-50 p-4 rounded-lg border border-gray-700">
                ...
                <h4 className="text-white font-medium mb-2">Register Your Dot</h4>
                <p className="text-gray-400 text-sm">Enter your name and join the battle royale as a unique dot.</p>
              </div>
              
              <div className="bg-gray-800 bg-opacity-50 p-4 rounded-lg border border-gray-700">
                <div className="text-cyan-400 text-xl font-bold mb-2">2</div>
                <h4 className="text-white font-medium mb-2">Watch the Battle</h4>
                <p className="text-gray-400 text-sm">Every night at 11:00 PM ET, dots face off in an epic battle.</p>
              </div>
              
              <div className="bg-gray-800 bg-opacity-50 p-4 rounded-lg border border-gray-700">
                <div className="text-cyan-400 text-xl font-bold mb-2">3</div>
                <h4 className="text-white font-medium mb-2">Claim Victory</h4>
                <p className="text-gray-400 text-sm">The last dot standing wins and earns a place on the leaderboard.</p>
              </div>
            </div>
          </div>
        </div>
