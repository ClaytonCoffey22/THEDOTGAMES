        ...
        <div className="text-center mb-8 pt-12 pb-8">
          ...
          <div className="mt-8 max-w-sm mx-auto bg-gray-800 rounded-lg p-4 border border-gray-700">
            ...
            <div className="font-mono text-2xl font-bold text-pink-500 mb-2">
              {timeUntilNextSimulation || "Loading..."}
            </div>
            <div className="text-sm text-gray-400">
              {nextSimulationTime && 
                `${nextSimulationTime.toLocaleDateString()} at ${nextSimulationTime.toLocaleTimeString()}`
              }
            </div>
          </div>
        </div>
            ...
            <div className="bg-gray-800 bg-opacity-50 rounded-lg p-4 border border-gray-700">
                ...
                <div className="flex space-x-2">
                  {!simulationState.inProgress && !simulationState.winner && (
                    <Button 
                      ...
                      disabled={simulationState.dots.length < 2}
                    >
                      Start Simulation
                    </Button>
                  )}
              ...
              <div className="mt-4 text-center">
                <p className="text-gray-400 text-sm">
                  {simulationState.dots.length < 2 ? (
                    <>
                      Not enough dots! <Link to="/join" className="text-cyan-400 hover:underline">Join the battle</Link> to add your dot.
                    </>
                    ...
                    <>
                      <span className="text-cyan-500 font-semibold">{simulationState.dots.length} dots</span> waiting for battle
                    </>
                  )}
                </p>
              </div>
